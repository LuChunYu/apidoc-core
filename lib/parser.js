...
Parser.prototype.parseFiles = function(options, parsedFiles, parsedFilenames) {
    ...
    for (var i = 0; i < files.length; i += 1) {
        ...
        var parsedFile = self.parseFile(filename, options);
        if (parsedFile) {
 ...
 * Execute Fileparsing
 */
Parser.prototype.parseFile = function(filename, options) {
    var self = this;
    ...
    // just use options.str insteadof read file
    if (options.str) {
      self.src = options.str
    } else {
